...
OPT_SAT        = true;               % velocity saturation
satX           = 0.15;             % value of the saturation x translation
satZ           = 0.2;              % y translation
satTheta       = 0.2;              % Z rotation
OPT_WARMING    = true;               % affine increase of the velocity during the 
                                   % the double support pahse
OPT_CORR       = true;               % correction of the error
% ----- For Walking
OPT_WALK_SIMU  = false;               % sinusoidal addition
OPT_WALK_HRP2  = true;               % walk simulation 
OPT_DISTURB    = false;               % addition of a disturbance
...
wMr            = computeTransfMatrix(poserInit);
wMrdes         = computeTransfMatrix(poserFinal);
...
while( iter < maxiter)
  ...
  %p          = p +(rand(p,'normal')-1/2)/100;
  e          = p-pdes;                                     % compute the error  
  norme      = [norme;norm(e)];                            % store the norm for display
   
   
  
   
  %------ SWAY MOTION COMPENSATION--------%/ 
   
   
  esav       = esav + dt*L(:,[1,3,5])*(vcamReal([1,3,5])-vcam)'; % error correction +=dt*L*(vPG-vCam)
  ...
  E =[E;norm(e-esav)'];  % store e-esav, ie error corrected
